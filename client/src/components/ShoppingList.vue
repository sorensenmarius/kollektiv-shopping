      ...
      <section class="main" v-show="items.length" v-cloak>
        ...
        <ul class="todo-list">
          <li
            ...
            class="todo"
            :key="item.id"
            :class="{ checked: item.checked, editing: item == editedItem }"
          >
            <div class="view">
              <input class="toggle" type="checkbox" v-model="item.checked" @click="toggleItemCheck(item)"/>
              <label @click="editItem(item)">{{ item.name }}</label>
...
export default {
    ...
    methods: {
        ...
        getItems: function() {
            let self = this
            funcs.httpsCallable('getItems')().then(function(res) {
                self.items = res.data
            })
        },
        ...
        doneEdit: function(item) {
            ...
            funcs.httpsCallable('updateItem')(item)
        },
        deleteItem: function(item) {
            let itemIndex = this.items.findIndex(currentItem => {
            ...
            if(itemIndex >= 0) {
                ...
                funcs.httpsCallable('deleteItem')({ id: item.id })
            }
        },
        toggleItemCheck: function(item) {
            item.checked = !item.checked
            funcs.httpsCallable('checkItem')({ id: item.id, checked: item.checked })
        },
